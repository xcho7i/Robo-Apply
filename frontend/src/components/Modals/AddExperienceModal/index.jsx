...
import { improveJobDescriptionWithAI } from "../../../api/functions"
...
const AddExperienceModal = ({
  ...
  const handleSaveExperience = () => {
    ...
    if (initialData.editIndex !== undefined) {
      onSave(initialData.editIndex, experienceData)
    } else {
  ...
  const handleAddToDescription = () => {
    ...
    if (description && description.trim() !== "") {
      ...
      combinedDescription = description + aiGeneratedDescription
    } else {
  ...
  const handleAISuggestion = async () => {
    ...
    try {
      ...
      const response = await improveJobDescriptionWithAI(payload)

      if (response.success && response.data && response.data.improved) {
    ...
    <>
      ...
      <div
        ...
        <div className="bg-modalPurple rounded-lg p-4 md:p-8 w-full max-w-[90%] md:max-w-[50%] mt-10 relative border max-h-[90vh] overflow-y-auto">
          <Button
            onClick={onClose}
            className="absolute top-3 right-3 bg-gradient-to-b rounded-full p-0.5 text-primary from-gradientStart to-gradientEnd">
            <IoMdClose size={24} />
          </Button>
          ...
          <div className="flex justify-end mt-5 mb-2 space-x-4 ">
            <Button
              onClick={handleClose}
              className="px-4 py-3 font-medium bg-gradient-to-b min-w-max w-40 from-gradientStart to-gradientEnd text-primary rounded-lg">
              Close
            </Button>
            <Button
              onClick={handleSaveExperience}
              className="px-4 py-3 font-medium bg-gradient-to-b min-w-max w-40 from-gradientStart to-gradientEnd text-primary rounded-lg">
              {initialData.editIndex !== undefined ? "Save Changes" : "Add Experience"}
            </Button>
          </div>
        </div>
      </div>
