...
const StepTwentyFour: React.FC<StepTwentyFourProps> = ({
  onboardingData,
  setOnboardingData,
  onNextStep,
  onPreviousStep
  ...
  const imagesrc = "/images/slide-24-image.png"
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      console.error("Step 24 Error:", err)
    } finally {
            ...
            <div className="w-full flex flex-col md:flex-row p-10 z-50">
              <div className="w-full md:w-1/2 flex flex-col justify-center gap-5 p-5">
                <div className="text-2xl text-white  font-bold">
                  Create Bulk tailored resumes for every job —
                  <span className="text-brand-yellow"> instantly</span>
                </div>
                <FormPara>
                  RoboApply tailors your resume to each job — no edits needed.
                  Show recruiters exactly what they're looking for.
                </FormPara>
              </div>
              <div className="w-full md:w-1/2 flex flex-col items-center justify-center">
                <img
                  src={imagesrc}
                  alt="Icon"
                  className="md:-mb-10 md:scale-125"
                />
              </div>
            </div>
          </FormCardLayout>
